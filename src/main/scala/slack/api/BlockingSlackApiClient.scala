...
object BlockingSlackApiClient {
  ...
  def apply(endpoint: String, token: String, duration: FiniteDuration = 5.seconds): BlockingSlackApiClient =
  ...
  def exchangeOauthForToken(endpoint: String,
    ...
    Await.result(SlackApiClient.exchangeOauthForToken(endpoint, clientId, clientSecret, code, redirectUri), duration)
}

import SlackApiClient._

class BlockingSlackApiClient(endpoint: String, token: String, duration: FiniteDuration = 5.seconds) {
  ...
  def postChatMessage(channelId: String,
    ...
    resolve(client.postChatMessage(channelId, text, username, asUser, parse, linkNames, attachments, unfurlLinks,
        unfurlMedia, iconUrl, iconEmoji))
  ...
  def getFileInfo(fileId: String, count: Option[Int] = None, page: Option[Int] = None)(
      implicit system: ActorSystem): FileInfo =
